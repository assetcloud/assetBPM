...
angular.module('flowableModeler').controller('FlowableRolePopupCtrl',
    [ '$rootScope', '$scope', '$translate', '$http', 'UserService', 'GroupService', function($rootScope, $scope, $translate, $http, UserService, GroupService) {
        ...
        if ($scope.property.rolenames !== undefined && $scope.property.rolenames !== null) {
            $scope.rolenames = $scope.property.rolenames;
        }else {
            $scope.property.rolenames = {};
            $scope.property.roleids = {};
        }

        $scope.save = function () {
            $scope.property.value = {};
            $scope.property.value.rolenames = jQuery('#frolenames').val();
            $scope.property.value.roleids = jQuery('#froleids').val();
